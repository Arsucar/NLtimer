...
@Singleton
class BulkUpdateTagsTool @Inject constructor(
    private val tagRepository: TagRepository,
    private val toolConfig: ToolConfig,
) : ToolDefinition {

    override val name: String = "bulkUpdateTags"
    override val description: String =
        "批量修改标签属性（图标、颜色、名称、分类），每个标签可独立指定不同字段，仅更新传入字段"
    ...
    override val parameters: List<ToolParameter> = listOf(
        ToolParameter(
            name = "updates",
            description = "更新列表，每个元素含 id(必填)、iconKey(可选)、color(可选)、name(可选)、category(可选)，最多 $MAX_BATCH_SIZE 条",
    ...
    override suspend fun execute(args: Map<String, Any?>): ToolResult {
        ...
        for (item in rawList) {
            ...
            val existing = tagRepository.getById(id)
            if (existing == null) {
                notFound.put(JSONObject().put("id", id).put("reason", "标签不存在"))
                continue
            }
            ...
            val merged = existing.copy(
                ...
                category = if (item.containsKey("category")) (item["category"] as? String) else existing.category,
            )

            runCatching {
                tagRepository.update(merged)
                updated.put(id)
            }.onFailure {
    ...
    override fun getDocumentation(): ToolDocumentation = ToolDocumentation(
        ...
        usageExamples = listOf(
            """bulkUpdateTags(updates=[{"id":1,"iconKey":"⚡","color":4278190335}])""",
            """bulkUpdateTags(updates=[{"id":2,"name":"新名称","category":"工作"}])""",
        ),
    )
