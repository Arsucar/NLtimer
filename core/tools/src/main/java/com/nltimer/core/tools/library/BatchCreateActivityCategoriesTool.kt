...
@Singleton
class BatchCreateActivityCategoriesTool @Inject constructor(
    private val categoryRepository: CategoryRepository,
    private val activityRepository: ActivityRepository,
    private val toolConfig: ToolConfig,
) : ToolDefinition {

    override val name: String = "batchCreateActivityCategories"
    override val description: String =
        "批量创建活动分类；传入分类名列表，自动去重，返回 created/skipped 结果"
    ...
    override suspend fun execute(args: Map<String, Any?>): ToolResult {
        ...
        val existingNames = activityRepository.getAllGroups().first()
            .map { it.name }
            .toSet()
        ...
        for (raw in rawList) {
            ...
            if (name in existingNames) {
                skipped.put(JSONObject().put("name", name).put("reason", "分类已存在"))
                continue
            }
            ...
            runCatching {
                ...
                skipped.put(JSONObject().put("name", name).put("reason", it.message ?: "创建失败"))
            }
        }
    ...
    override fun getDocumentation(): ToolDocumentation = ToolDocumentation(
        ...
        usageExamples = listOf(
            """batchCreateActivityCategories(names=["运动","学习","生活"])""",
        ),
    )
